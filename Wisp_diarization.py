...
from lang import get_text as tx, LANG
...
class MainWindow(QWidget):
    def __init__(self, *args, **kwargs):
        ...
        # Create a button that allows to choose if the audio should be diarized
        diarization_button = QPushButton(tx("Diarization_selection"))
        diarization_button.clicked.connect(self.choose_diarization)
        # Display default option at the start. Display choosen option if the user changes it.
        self.diarization_edit = QLineEdit()
        self.filepath_edit = QLineEdit()
        self.diarization_edit.setText(tx("Default_diarization"))
        ...
        layout.addWidget(diarization_button, 10, 0)
        layout.addWidget(self.diarization_edit, 11, 0)
        layout.addWidget(translation_button, 12, 0)
        layout.addWidget(self.translation_edit, 13, 0)

        # Add the button to start the transcription process
        layout.addWidget(file_transcribe, 14, 0)
        layout.addWidget(self.transcription_edit, 15, 0)
        layout.addWidget(close_button, 16, 0)
    ...
    def choose_diarization(self):
        "This method allows the user to choose if the text should be diarized - if different speakers should be recognized."
        # Define options: Yes or No.
        items = [tx("No"), tx("Yes")]

        # Create field to allow user to choose
        item, ok =  QInputDialog.getItem(self, tx("Diarization_setting"), tx("Text_Diarization_setting"), items, 0, False)
        ...
        if ok and item == tx("Yes"):
            self.diarization = 1
            self.diarization_edit.setText(tx("Yes"))
        if ok and item == tx("No"):
            self.diarization = 0
            self.diarization_edit.setText(tx("No"))
    ...
    def transcribe(self):
        ...
        # One could use float16 if the computer supports it. All computers support int8.
        compute_type = "int8"
        ...
        if filesize > 0:
            ...
            try:
                if diarize == 1:
                    ...
                    result = whisperx.align(result["segments"], model_a, metadata, filename, device, return_char_alignments=False)
                    ...
                    print("Result segments: ",result["segments"]) # segments are now assigned speaker IDs
